...
The following example matches a request to the `/api/content` endpoint that provides `selector`, `page` and `pageSize` query parameters
that matches exactly the specified values. A response with status code `200` and a JSON body will bre returned.
The content of this response is located in `api/content/get_list_body.json` file located in the `__files` folder.
...
----
{
    ...
    "response": {
        ...
        "bodyFileName": "api/content/get_list_body.json"
    }
}
----
